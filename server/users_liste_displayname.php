...
$displayname = trim(isset($_REQUEST['displayname'])?$_REQUEST['displayname']:'');
$prm_nostar = str_replace('*', '', $displayname);
if ($displayname != '' && strlen($prm_nostar) >= 3) {
    $displayname = rawurldecode($displayname);
    if (substr($displayname, strlen($displayname)-1, 1) != '*') {
        $displayname .= '*';
    }
    $ad = ls_ldap_connect();
    if ($ad !== false) {
